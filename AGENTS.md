...
- The local adapter and the hosted service both expose `find_ui_references` and `find_ui_materials`. Keep those two catalogue contracts identical, including limits (1–3, default 2), the `selectedIds` inspection path, the `pack` refusal, and plain-text empty results.
